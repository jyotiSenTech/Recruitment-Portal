            ...
            <div class="card">
                ...
                <div class="table-responsive container" style="margin-top: 20px; margin-bottom: 20px;">
                    <table id="datatable" class="table table-striped table-bordered cell-border">
                        ...
                        <tbody>
                            @foreach ($verified_lists as $index => $verified_list)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $verified_list->First_Name }}</td>
                                <td>{{ $verified_list->Contact_Number }}</td>
                                <td>{{ $verified_list->Last_Updated_dttime }}</td>
                                <td>
                                    <a href="{{ url('/admin/view-application-detail/'.md5($verified_list->Applicant_ID)) }}">
                                        <button class="btn btn-info btn-sm">View Application</button>
                                    </a>
                                </td>
                                <td>
                                    @if($verified_list->Application_Status == 'Submitted')
                                    <strong style="color: gray">Submitted</strong>
                                    @elseif($verified_list->Application_Status == 'Rejected')
                                    <strong style="color: red">Not Eligible</strong>
                                    @elseif($verified_list->Application_Status == 'Verified')
